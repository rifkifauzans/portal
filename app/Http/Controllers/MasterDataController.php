...
{
    public function getkodekebunfilter($id = null){
        $month = Carbon::now()->format('m');
        $year = Carbon::now()->format('Y');
        $datauser = "";

        $datauser= DB::connection('sqlsrv')->table('File_A1_SQL')->where('KD_KBN',$id)->where('STS_ADMINISTRATIF','AKTIF')->wheremonth('STAT_REC',$month)->whereyear('STAT_REC',$year)->orderby('nama')->get();
